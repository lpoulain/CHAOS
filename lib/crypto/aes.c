...
static const uint8 aes_sbox[16][16] = {
...
static const uint8 aes_invsbox[16][16] = {
...
static const uint8 gf_mul[256][6] = {
...
// XORs the in and out buffers, storing the result in out. Length is in uint8s.
void xor_buf(const uint8 in[], uint8 out[], uint len)
{
	uint idx;
...
// Substitutes a uint using the AES S-Box.
uint Subuint(uint uint)
{
	unsigned int result;

	result = (int)aes_sbox[(uint >> 4) & 0x0000000F][uint & 0x0000000F];
	result += (int)aes_sbox[(uint >> 12) & 0x0000000F][(uint >> 8) & 0x0000000F] << 8;
	result += (int)aes_sbox[(uint >> 20) & 0x0000000F][(uint >> 16) & 0x0000000F] << 16;
	result += (int)aes_sbox[(uint >> 28) & 0x0000000F][(uint >> 24) & 0x0000000F] << 24;
	return(result);
}
...
void aes_key_setup(const uint8 key[], uint w[], int keysize)
{
	...
	for (idx = Nk; idx < Nb * (Nr+1); ++idx) {
		temp = w[idx - 1];
		if ((idx % Nk) == 0)
			temp = Subuint(KE_ROTWORD(temp)) ^ Rcon[(idx-1)/Nk];
...
// Performs the AddRoundKey step. Each round has its own pre-generated 16-uint8 key in the
...
void AddRoundKey(uint8 state[][4], const uint w[])
{
	uint8 subkey[4];
...
// Performs the Subuint8s step. All uint8s in the state are substituted with a
// pre-calculated value from a lookup table.
void Subuint8s(uint8 state[][4])
{
...
void InvSubuint8s(uint8 state[][4])
{
...
void aes_encrypt(const uint8 in[], uint8 out[], const uint key[], int keysize)
{
	uint8 state[4][4];

	// Copy input array (should be 16 uint8s long) to a matrix (sequential uint8s are ordered
	...
	Subuint8s(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[4]);
	Subuint8s(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[8]);
	Subuint8s(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[12]);
	Subuint8s(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[16]);
	Subuint8s(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[20]);
	Subuint8s(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[24]);
	Subuint8s(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[28]);
	Subuint8s(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[32]);
	Subuint8s(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[36]);
	if (keysize != 128) {
		Subuint8s(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[40]);
		Subuint8s(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[44]);
		if (keysize != 192) {
			Subuint8s(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[48]);
			Subuint8s(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[52]);
			Subuint8s(state); ShiftRows(state); AddRoundKey(state,&key[56]);
		}
		else {
			Subuint8s(state); ShiftRows(state); AddRoundKey(state,&key[48]);
		}
	}
	else {
		Subuint8s(state); ShiftRows(state); AddRoundKey(state,&key[40]);
	}
...
void aes_decrypt(const uint8 in[], uint8 out[], const uint key[], int keysize)
{
	uint8 state[4][4];
	...
	if (keysize > 128) {
		if (keysize > 192) {
			...
			InvShiftRows(state);InvSubuint8s(state);AddRoundKey(state,&key[52]);InvMixColumns(state);
			InvShiftRows(state);InvSubuint8s(state);AddRoundKey(state,&key[48]);InvMixColumns(state);
		}
		else {
			AddRoundKey(state,&key[48]);
		}
		InvShiftRows(state);InvSubuint8s(state);AddRoundKey(state,&key[44]);InvMixColumns(state);
		InvShiftRows(state);InvSubuint8s(state);AddRoundKey(state,&key[40]);InvMixColumns(state);
	}
	else {
		AddRoundKey(state,&key[40]);
	}
	InvShiftRows(state);InvSubuint8s(state);AddRoundKey(state,&key[36]);InvMixColumns(state);
	InvShiftRows(state);InvSubuint8s(state);AddRoundKey(state,&key[32]);InvMixColumns(state);
	InvShiftRows(state);InvSubuint8s(state);AddRoundKey(state,&key[28]);InvMixColumns(state);
	InvShiftRows(state);InvSubuint8s(state);AddRoundKey(state,&key[24]);InvMixColumns(state);
	InvShiftRows(state);InvSubuint8s(state);AddRoundKey(state,&key[20]);InvMixColumns(state);
	InvShiftRows(state);InvSubuint8s(state);AddRoundKey(state,&key[16]);InvMixColumns(state);
	InvShiftRows(state);InvSubuint8s(state);AddRoundKey(state,&key[12]);InvMixColumns(state);
	InvShiftRows(state);InvSubuint8s(state);AddRoundKey(state,&key[8]);InvMixColumns(state);
	InvShiftRows(state);InvSubuint8s(state);AddRoundKey(state,&key[4]);InvMixColumns(state);
	InvShiftRows(state);InvSubuint8s(state);AddRoundKey(state,&key[0]);
...
int aes_encrypt_cbc(const uint8 in[], uint len, uint8 out[], const uint8 key[], int keysize, const uint8 iv[]) {
	...
	uint8 buf_in[AES_BLOCK_SIZE], buf_out[AES_BLOCK_SIZE], iv_buf[AES_BLOCK_SIZE];
	int blocks, idx;
